...
static void OnJoinLedTimerEvent(void *context);
...
static void SendTxData(void) {
	...
	if (isPending == 1) {
		...
		AppDataBuffer[0] = (uint8_t)'v';
		AppDataBuffer[1] = (uint8_t)FW_VER_MAJOR;
		AppDataBuffer[2] = (uint8_t)'.';
		AppDataBuffer[3] = (uint8_t)FW_VER_MINOR;
		AppDataBuffer[4] = (uint8_t)'.';
		AppDataBuffer[5] = (uint8_t)FW_VER_REV;
		AppDataBuffer[6] = (run_maxbotix_test==1)?(uint8_t)'P': (uint8_t)'F';
		AppDataBuffer[7] = (run_flash_test==1)?(uint8_t)'P': (uint8_t)'F';
		uint8_t *print_UID;
		print_UID = flash_UID;
		for (int i=0; i<8; i++)
			AppDataBuffer[8+i] = (uint8_t)*print_UID++;
		AppDataBuffer[16] = (uint8_t)flash_pct;
		
		LmHandlerAppData_t appData = {
			.Buffer = AppDataBuffer, .BufferSize = 16, .Port = 1};
...
static void OnJoinLedTimerEvent(void *context){
	LmHandlerFlagStatus_t joinStatus;
	if ((joinStatus = LmHandlerJoinStatus()) == LORAMAC_HANDLER_SET){
		HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, GPIO_PIN_SET);
	}
	else {
