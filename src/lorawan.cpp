...
void JoinFailureDebug(void) {
    if(JoinFailed() && useOrigApp == 'n') {
        ...
        // Use old AppEUI
        useOrigApp = 'y';
        EEPROM.write(selectionAddress, (byte)(useOrigApp-'0'));
...
void ModifyAppEUI(void){
    for (int i = 0; i < 8; ++i)
    {
        EEPROM.write(newAppAddress + i, mcpsIndication->Buffer[i+1]); //big-endian
    }
    useOrigApp = 'n'; // Use new AppEUI on restart
    EEPROM.write(selectionAddress, (byte)(useOrigApp - '0')); // Save the choice to Flash Memory
    // restart
    innerWdtEnable(autoFeed);
}
    ...
    switch(mcpsIndication->Buffer[0]) {
        ...
        case 0x41:
            ...
            if (mcpsIndication->BufferSize == 9)
            {
                ...
                Serial.println("Invalid AppKey size!");
            }
            break;
        default:
...
void InitStoreAPPEUI(void){
    ...
    useOrigApp = 'y';
    init_ = true;
    int val = useOrigApp - '0';
    EEPROM.write(selectionAddress, val);
    // Save the default APPEUI in the Flash Memory
    for (int i = 0; i < 8; ++i)
    {
        EEPROM.write(defaultAppAddress + i, appEui[i]); //big-endian
    }
...
void setup_lorawan(unsigned int packet_interval) {
    ...
    enableAt();
#endif
    useOrigApp = (int)(EEPROM.read(selectionAddress)) + '0';
    if (useOrigApp == 'y' || useOrigApp == 'Y') {
        // First time setup...
        InitStoreAPPEUI();
    }
