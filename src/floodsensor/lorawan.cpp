...
const int MINOR_VERSION =
    2; // add functionality in a backwards compatible manner
...
uint16_t userChannelsMask[6] = {0xFF00, 0x0000, 0x0000,
                                0x0000, 0x0000, 0x0000}; // 0xFF00 from 00FF
...
/*LoRaWAN application data transmission duty cycle.  value in [ms]. DEFAULT is
 * 10 seconds*/
uint32_t appTxDutyCycle = 10 * 1000;

/*User defined variable - TX_INTERVAL, changes between states as needed. DEFAULT
 * is 10 seconds*/
...
volatile static uint16_t tip_count;
...
void ModifyKeys(McpsIndication_t *mcpsIndication) {
  ...
  for (int i = 0; i < 8; ++i) {
    EEPROM.write(newAppAddress + i,
                 byte(mcpsIndication->Buffer[i + 1])); // big-endian
  }
  for (int i = 0; i < 8; ++i) {
    EEPROM.write(newDevEUIAddress + i,
                 byte(mcpsIndication->Buffer[i + 9])); // big-endian
  }
  for (int i = 0; i < 16; ++i) {
    EEPROM.write(newAppKeyAddress + i,
                 byte(mcpsIndication->Buffer[i + 17])); // big-endian
  }
...
uint8_t get_current_sensor_state(void) { return SENSOR_STATE; }
...
void process_operation(McpsIndication_t *mcpsIndication) {
  if ((char(mcpsIndication->Buffer[1]) == 's') &&
      (char(mcpsIndication->Buffer[2]) == 't') &&
      (char(mcpsIndication->Buffer[3]) == 'a') &&
      (char(mcpsIndication->Buffer[4]) == 'r') &&
      (char(mcpsIndication->Buffer[5]) == 't')) {
    ...
    if (is_deployed == 'y') {
      ...
      Serial.println("Saved the sensor state. From now uplinks will be "
                     "readings even after reset.....");
      EEPROM.end();
    }
    SENSOR_STATE = SENSING_STATE;
    appTxDutyCycle = TX_INTERVAL * 1000;
  } else if ((char(mcpsIndication->Buffer[1]) == 's') &&
             (char(mcpsIndication->Buffer[2]) == 't') &&
             (char(mcpsIndication->Buffer[3]) == 'o') &&
             (char(mcpsIndication->Buffer[4]) == 'p')) {
    Serial.println("Stop sensing, uplinks are sensor CFG packets.");
    SENSOR_STATE = CFG_STATE;
    appTxDutyCycle = 10 * 1000;
  } else if ((char(mcpsIndication->Buffer[1]) == 'r') &&
             (char(mcpsIndication->Buffer[2]) == 'e') &&
             (char(mcpsIndication->Buffer[3]) == 's') &&
             (char(mcpsIndication->Buffer[4]) == 'e') &&
             (char(mcpsIndication->Buffer[5]) == 't')) {
...
void downLinkDataHandle(McpsIndication_t *mcpsIndication) {
  ...
  Serial.println();
  uint32_t color = mcpsIndication->Buffer[0] << 16 |
                   mcpsIndication->Buffer[1] << 8 | mcpsIndication->Buffer[2];
#if (LoraWan_RGB == 1)
  ...
  case 0x4D:
    /* Sensor Mode Change
       Packet format:
      |   oper     | Duty Cycle in seconds  | Sensor Mode | Sampling Rate  |
      Number of readings per measurement  | |   0x4D     |       2 bytes |    1
      byte   |    2 bytes     |        1 byte                        |
     */
  ...
  case 0x41:
    /* Sensor APP Change
       Packet format:
      |   oper     |   AppEUI(big-endian)   |   DevEUI(big-endian)  |
      AppKEY(big-endian)    | |   0x41     |         8 bytes        |          8
      bytes      |         16 bytes        |
     */
...
void prepareMaxbotixFrame() {
  // Regular Uplink contains: Sensor Error Flags followed by Battery and then
  // Sensor Data
  ...
  /*
   ...
   |----------------------------------------------Error Flags
   ------------------------------------------------------| |     bit 7 |  bit 6
   |  bit 5  |  bit 4  |  bit 3  |  bit 2  |  bit 1  |      bit 0       | | Used
   only for CFG update    |          |         |         |         |         |
   |   SD error flag  |
   */
  ...
  // Maxbotix
  distance = read_sensor_using_modes(sensorMode, sensor_sampling_rate,
...
static void prepareTxFrame(uint8_t port) {
  /*appData size is LORAWAN_APP_DATA_MAX_SIZE which is defined in
     "commissioning.h". appDataSize max value is LORAWAN_APP_DATA_MAX_SIZE. if
     enabled AT, don't modify LORAWAN_APP_DATA_MAX_SIZE, it may cause system
     hanging or failure. if disabled AT, LORAWAN_APP_DATA_MAX_SIZE can be
     modified, the max value is reference to lorawan region and SF. for example,
     if use REGION_CN470, the max value for different DR can be found in
...
// #endif
  ...
  case CFG_STATE: {
    /*
     ...
     | Error Flag  |   sensor_sleep   |    sensor_agg     |   sensor_meas_delta
     | sensor_reading_count   |    sensor_state   |    fw_ver       | |    255
     (FF) |     2 bytes      |      1 byte       |          2 bytes        | 1
     byte        |        1 byte     |    6 bytes      |
...
void joinTimedOutEvent(void) {
  ...
  //  reset the MCU
  Serial.println(
      "Join TimedOut, resetting the MCU will join default TTN Application..");
  innerWdtEnable(false);
  delay(5000); // Wait until the MCU resets
}
...
void setup_lorawan(unsigned int packet_interval) {
  ...
  */
  PINMODE_INPUT_PULLDOWN(ADC1);
  attachInterrupt(ADC1, tipIncrease, CHANGE);
  ...
  EEPROM.end();
  Serial.println(F(
      "Checking previously known sensor state and setting up dutycycle....."));
  if (is_deployed == 'y') {
    Serial.println(
        "\n\n=========================================================");
    Serial.println("Sensor is deployed!\n");
    Serial.print("Reset cause CY_SYS_RESET_WDT: ");
    Serial.println(CY_SYS_RES_CAUSE_REG & (CY_SYS_RESET_WDT)); // If True - 1
    Serial.print("Reset cause CY_SYS_RESET_PROTFAULT: ");
    Serial.println(CY_SYS_RES_CAUSE_REG &
                   (CY_SYS_RESET_PROTFAULT)); // If true - 8
    Serial.print("Reset cause CY_SYS_RESET_SW: ");
    Serial.println(CY_SYS_RES_CAUSE_REG & (CY_SYS_RESET_SW)); // If true - 16
...
void lorawan_runloop_once(void) {
  ...
  case DEVICE_STATE_CYCLE: {
    ...
    Serial.print("Going to sleep, next uplink in ");
    Serial.print(TX_INTERVAL);
    Serial.println(" s.");
