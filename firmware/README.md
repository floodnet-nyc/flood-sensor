...
Add HelTec Boards to the IDE, there are two ways:
 - Using Arduino's Boards Manager.
     - Open Arduino IDE, and goto File->Preferences->Settings img1
     ...
     - After that goto Tools->Board:->Boards Manager, search CubeCell in the new pop-up dialog, select the latest releases and click install. img2.
 - Using Github.
...
### Creating an end-device
 Registering End Device.
   ...
   - Select the end device in the LoRaWAN Device Repository. Refer the following image. -img3
   - In JoinEUI an ID has to be entered which can be anything (for example, FA CD 67 98 HY 7J 54 00). Once entered, additional DevEUI and AppKey have to be
     entered which can be generated by clicking on generate. And then click on Register end Device.  Refer to the following image. 
...
### Installing the device secrets
 - Connecting CubeCell to the Arduino IDE.
    - Open Arduino IDE and goto Tools>Board>CubeCell>HTTC(AB02). And set the configurations according to the image below. -img4
 ...
 - Adding device secerts in the firmware.
    ...
    - Enter the DevEUI, JoinEUI and AppKey in TTN_DEVEUI, TTN_APPEUI AND TTN_APPKEY respectively. Refer the image. img5
